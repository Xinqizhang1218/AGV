...
typedef void* GX_WIND_HANDLE;							///< \Chinese 窗口句柄                                                                                       \English window Handle
typedef void* GX_OPERATE_HANDLE;						///< \Chinese 对创建成功窗口的操作句柄                                                                       \English operate window Handle
typedef uint64_t void_64;
enum GX_AUTO_CONNECTION_STATUS_ENTRY;

typedef enum GX_WINDOWS_ID
{
    GX_WND_PROP = 0,                                    ///< 显示属性窗口类型	              \English The displayed window is of the camera properties type.
}GX_WINDOW_TYPE;

typedef enum GX_WINDOWS_SHOW_MODE
{
	NON_BLOCK_SHOW_MODE = 0,                            ///< 以非阻塞模式运行窗口               \English Run the window in non-blocking mode
	BLOCK_SHOW_MODE = 1,                                ///< 以阻塞模式运行窗口	                \English Run the window in blocking mode
