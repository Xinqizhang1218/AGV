...
/**
\brief      保存图像
\param[in]  stSaveImageInfo    图像信息
\retrun     GX_API

\English:
\brief      Save image.
\param		[in]stSaveImageInfo			  Image information
\return     GX_STATUS_SUCCESS             The operation is successful and no error occurs.
			GX_STATUS_NOT_INIT_API        The GXInitLib initialization library is not called.
			GX_STATUS_INVALID_PARAMETER   The pointer that the user input is NULL.
			The errors that are not covered above please reference GX_STATUS_LIST
*/
// ---------------------------------------------------------------------------
GX_API GXSaveImage(GX_SAVE_IMAGE_INFO* stSaveImageInfo);

// ---------------------------------------------------------------------------
/**
\brief          添加视频帧进行视频录制
\param[in]      phRecorder    视频句柄
\param[in]      pImageBuffer    图像数据
\retrun         GX_API

\English:
\brief          Create a video handle and record the video
\param[in]      stRecordParam   Video information
\param[in]      phRecorder    video handle
\return			GX_STATUS_SUCCESS             The operation is successful and no error occurs.
				GX_STATUS_NOT_INIT_API        The GXInitLib initialization library is not called.
				GX_STATUS_INVALID_PARAMETER   The pointer that the user input is NULL.
				The errors that are not covered above please reference GX_STATUS_LIST
*/
// ---------------------------------------------------------------------------
GX_API GXCreateRecorder(GX_RECORD_PARAM* stRecordParam, GX_RECORDER_HANDLE* phRecorder);
...
*/
// ---------------------------------------------------------------------------
GX_API GXAddFrame(GX_RECORDER_HANDLE phRecorder, unsigned char * pImageBuffer);
...
\English:
\brief          Stop recording and delete the video handle
\param[in]      phRecorder    video handle
\return			GX_STATUS_SUCCESS             The operation is successful and no error occurs.
				GX_STATUS_NOT_INIT_API        The GXInitLib initialization library is not called.
				GX_STATUS_INVALID_PARAMETER   The pointer that the user input is NULL.
...
*/
// ---------------------------------------------------------------------------
GX_API GXDestroyRecorder(GX_RECORDER_HANDLE phRecorder);
...
\brief      创建窗口操作句柄
\param      [in]hDevice                   设备句柄
\param      [in]emID                      窗口的不同类型,目前仅有属性窗口
\param      [in]hParentWnd                调用时此参数必需传NULL，窗口将会弹出显示。（预留此参数后面支持嵌入显示）
\param      [out]hWnd                     当前创建成功的窗口句柄
...
\English:
\brief      Create a window based on the provided serial port ID and type.
\param      [in]hDevice                   The handle of the device
\param      [in]emID                      Different Types of Windows Currently Only Property Window
\param      [in]hParentWnd                his parameter requires passing NULL when called, as the window will pop up for display.
                                         (This parameter is reserved to support embedded display in the future.)
\param      [out]hWnd                     Successfully created window handle
...
*/
// ---------------------------------------------------------------------------
GX_API GXCreateWnd(GX_DEV_HANDLE hDevice, GX_WINDOWS_ID emID, GX_WIND_HANDLE hParentWnd, GX_OPERATE_HANDLE* hWnd);
...
\param      [in]hDevice                   The handle of the windows
\return     GX_STATUS_SUCCESS             The operation is successful and no error occurs.
            GX_STATUS_NOT_INIT_API        The GXInitLib initialization library is not called.
            GX_STATUS_INVALID_PARAMETER   The pointer that the user input is NULL.
            The errors that are not covered above please reference GX_STATUS_LIST.
*/
// ---------------------------------------------------------------------------
GX_API GXDestroyWnd(GX_OPERATE_HANDLE hWnd);

// ---------------------------------------------------------------------------
/**
\Chinese：
\brief      设置窗口位置
\param      [in]hWnd                      窗口操作句柄
\param      [in]nPosX                     窗口水平位置
\param      [in]nPosY                     窗口垂直位置
\param      [in]nWidth                    窗口宽（为了正确显示，必须大于等于450）
\param      [in]nHeight                   窗口高（为了正确显示，必须大于等于600）
\return     GX_STATUS_SUCCESS             操作成功，没有发生错误
            GX_STATUS_NOT_INIT_API        没有调用GXInitLib初始化库
            GX_STATUS_INVALID_PARAMETER   用户输入的指针为NULL
            其它错误情况请参见GX_STATUS_LIST
...
\param      [in]nWidth                    Width of the windows(To display correctly, it must be greater than or equal to 665.)
...
*/
// ---------------------------------------------------------------------------
GX_API GXSetShowPosition(GX_OPERATE_HANDLE hWnd, int32_t nPosX, int32_t nPosY, int32_t nWidth, int32_t nHeight);
...
\brief      设置窗口显示模式
\param      [in]hWnd                      窗口句柄
\param      [in]emMode                    窗口模式，当设置为NON_BLOCK_SHOW_MODE时不会阻塞后面要执行的操作；
                                                  当设置为BLOCK_SHOW_MODE时会阻塞后面要执行的操作。
                                                  注意：如果您的程序是GUI程序如（Qt、MFC、Winforms等）请勿调用。
\return     GX_STATUS_SUCCESS             操作成功，没有发生错误
            GX_STATUS_NOT_INIT_API        没有调用GXInitLib初始化库
            GX_STATUS_INVALID_PARAMETER   用户输入的指针为NULL
            其它错误情况请参见GX_STATUS_LIST
...
*/
// ---------------------------------------------------------------------------
GX_API GXSetShowMode(GX_OPERATE_HANDLE hWnd, GX_WINDOWS_SHOW_MODE emMode);
// ---------------------------------------------------------------------------
/**
\Chinese：
\brief      显示窗口
\param      [in]hWnd                      窗口句柄
\param      [in]bVisible                  true：显示，fals：隐藏
\return     GX_STATUS_SUCCESS             操作成功，没有发生错误
            GX_STATUS_NOT_INIT_API        没有调用GXInitLib初始化库
            GX_STATUS_INVALID_PARAMETER   用户输入的指针为NULL
            其它错误情况请参见GX_STATUS_LIST
...
\param      [in]bVisible                 true: show window, false: hide window
\return     GX_STATUS_SUCCESS             The operation is successful and no error occurs.
            GX_STATUS_NOT_INIT_API        The GXInitLib initialization library is not called.
            GX_STATUS_INVALID_PARAMETER   The pointer that the user input is NULL.
            The errors that are not covered above please reference GX_STATUS_LIST.
*/
// ---------------------------------------------------------------------------
GX_API GXShowWnd(GX_OPERATE_HANDLE hWnd, bool bVisible);
...
\param      [in]hWnd                      The handle of the window
\param      [in]strTitle                  window title
\return     GX_STATUS_SUCCESS             The operation is successful and no error occurs.
            GX_STATUS_NOT_INIT_API        The GXInitLib initialization library is not called.
            GX_STATUS_INVALID_PARAMETER   The pointer that the user input is NULL.
            The errors that are not covered above please reference GX_STATUS_LIST.
*/
// ---------------------------------------------------------------------------
GX_API GXSetWndTitle(GX_OPERATE_HANDLE hWnd, const char*  strTitle);
#endif  //GX_GALAXY_H
