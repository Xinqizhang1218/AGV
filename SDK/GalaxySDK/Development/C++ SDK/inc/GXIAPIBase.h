...
typedef enum GX_STATUS_LIST
{
	...
	GX_STATUS_LIBRARY_FAILED         = -15,			         ///< Load library error
	GX_STATUS_CREATE_WND_FAILED      = -16,			     ///< Create Window error
...
typedef struct GX_GIGE_ACTION_COMMAND_RESULT
{
    ...
    int32_t  nStatus;                         ///< GigE Vision status code returned by the device GX_ACTION_COMMAND_STATUS
...
typedef void* GX_WIND_HANDLE;                                       ///< The handle of the parent window
typedef void* GX_OPERATE_HANDLE;                                    ///< The handle of the operation window
typedef enum GX_WINDOWS_ID
{
	GX_WND_PROP = 0,                                                ///< The displayed window is of the camera properties type.
}GX_WINDOW_TYPE;

typedef enum GX_WINDOWS_SHOW_MODE
{
    NON_BLOCK_SHOW_MODE = 0,                                        ///< Run the window in non-blocking mode
    BLOCK_SHOW_MODE = 1,                                            ///< Run the window in blocking mode
}GX_WINDOWS_SHOW_MODE;
#endif //GXIAPI_BASE_H