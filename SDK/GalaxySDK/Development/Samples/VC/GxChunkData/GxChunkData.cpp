	...
	while (pThreadParam->bRun)
	{
		...
		if (GX_STATUS_SUCCESS == emStatus)
		{
			if (GX_FRAME_STATUS_SUCCESS == pFrameBuffer->nStatus)
			{
				...
				emStatus = GXGetIntValue((GX_CHUNK_DATA_HANDLE)pFrameBuffer->pChunkDataHandle, "ChunkFrameID", &stValue);	
				if (emStatus != GX_STATUS_SUCCESS)
				{
					GetErrorString(emStatus);
				}
				...
				cout << "<Successful acquisition: Width: " << pFrameBuffer->nWidth <<
					" Height: " << pFrameBuffer->nHeight << " ChunkFrameID: "<< stValue.nCurValue <<
					">" << endl;
			}
			else
			{
...
int main(int argc, char* argv[])
{
	...
	emStatus = GXGetNodeAccessMode(hDevice, "ChunkModeActive", &emAccessMode);
	GX_VERIFY_EXIT(emStatus, hDevice);

	//不支持帧信息的相机，都没有ChunkModeActive功能，因此无法输出帧信息,因此程序结束；
	if (emAccessMode == GX_NODE_ACCESS_MODE_NI || emAccessMode == GX_NODE_ACCESS_MODE_NA
		|| emAccessMode == GX_NODE_ACCESS_MODE_RO || emAccessMode == GX_NODE_ACCESS_MODE_UNDEF)
	{
		//关闭相机设备
		GXCloseDevice(hDevice);
		//关闭设备库
    ...
    emStatus = GXGetNodeAccessMode(hDevice, "ChunkSelector", &emAccessMode);
    if (emAccessMode == GX_NODE_ACCESS_MODE_WO || emAccessMode == GX_NODE_ACCESS_MODE_RW)
    {
        emStatus = GXSetEnumValueByString(hDevice, "ChunkSelector", "FrameID");
        GX_VERIFY_EXIT(emStatus, hDevice);
		emStatus = GXGetNodeAccessMode(hDevice, "ChunkEnable", &emAccessMode);
		GX_VERIFY_EXIT(emStatus, hDevice);
		if (emAccessMode == GX_NODE_ACCESS_MODE_WO || emAccessMode == GX_NODE_ACCESS_MODE_RW)
		{
