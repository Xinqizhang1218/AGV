...
class CProcessOpenError
{
public:
	CProcessOpenError(HWND hWnd, GX_DEV_HANDLE* pDeviceHandle, size_t nDevNum)
		: m_hWnd(hWnd)
	...
	~CProcessOpenError()
	{
		...
		{
			GX_STATUS errCode = 0;
			GX_STATUS lastErrCode = 0;
			char arrErrInfo[1024] = { 0 };
			size_t size = sizeof(arrErrInfo);
			errCode = GXGetLastError(&lastErrCode, arrErrInfo, &size);
			if ((GX_STATUS_SUCCESS == errCode) && (GX_STATUS_SUCCESS != lastErrCode))
			{
				MessageBox(m_hWnd, (LPCTSTR)arrErrInfo, "GxTransferDelay", MB_OK);
			}

			if (NULL == m_pDeviceHandle)
			{
				return;
			}
			...
			{
				if (NULL != m_pDeviceHandle[i])
				{
					GXCloseDevice(m_pDeviceHandle[i]);
	...
	void SetHasError(bool bHasError)
	{
		m_bHasError = bHasError;
	}

	HWND m_hWnd;
	GX_DEV_HANDLE* m_pDeviceHandle;
	size_t m_nDevNum;
	bool m_bHasError;
};
...
void CGxTransferDelayDlg::OnOpenDevice() 
{
	...
	CProcessOpenError autoShowErr(*this, m_pDeviceHandle, MAX_NUM_CAMERA);
