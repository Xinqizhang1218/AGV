...
void CGXBitmap::__ImageProcess(bool bIsColor, void * pImageBuffer, void * pImageProcess)
{
	...
	do {
		...
		if (!bIsRaw8)
		{
			...
			GX_PIXEL_FORMAT_ENTRY emDestFormat;
			emDestFormat = GetConvertDest8BitPixelFormat(static_cast<GX_PIXEL_FORMAT_ENTRY>(m_i64PixelFormat));
			...
			// 设置目标像素格式
			emDxStatus = (DX_STATUS)DxImageFormatConvertSetOutputPixelFormat(pImgFormatConvert, emDestFormat);
			DX_VERIFY(emDxStatus);
			...
			// 获取目标Buffer大小
			emDxStatus = (DX_STATUS)DxImageFormatConvertGetBufferSizeForConversion(pImgFormatConvert, emDestFormat
...
*/
//----------------------------------------------------------------------------------
void CGXBitmap::SaveRaw(void* pRawBuffer, int32_t nImgSize, const char* pFilePath)
{
	...
	{
		::WriteFile(hFile, pRawBuffer, nImgSize, &static_cast<DWORD>(bytes_read), NULL);
		CloseHandle(hFile);
	}
}
...
/**
\brief     获取转换的目标8位像素格式
\param     nPixelFormat是要转换的像素格式
\returen   转换的目标8位像素格式
*/
//----------------------------------------------------------------------------------
GX_PIXEL_FORMAT_ENTRY CGXBitmap::GetConvertDest8BitPixelFormat(GX_PIXEL_FORMAT_ENTRY nPixelFormat) const
{
	GX_PIXEL_FORMAT_ENTRY emDestPixelFormat = GX_PIXEL_FORMAT_UNDEFINED;
	switch (nPixelFormat)
	{
	case GX_PIXEL_FORMAT_BAYER_GR10_P:
	case GX_PIXEL_FORMAT_BAYER_GR12_P:
	...
	case GX_PIXEL_FORMAT_BAYER_GR8:
	{
		emDestPixelFormat = GX_PIXEL_FORMAT_BAYER_GR8;
		break;
	}
	case GX_PIXEL_FORMAT_BAYER_RG10_P:
	case GX_PIXEL_FORMAT_BAYER_RG12_P:
	...
	case GX_PIXEL_FORMAT_BAYER_RG8:
	{
		emDestPixelFormat = GX_PIXEL_FORMAT_BAYER_RG8;
		break;
	}
	case GX_PIXEL_FORMAT_BAYER_GB10_P:
	case GX_PIXEL_FORMAT_BAYER_GB12_P:
	...
	case GX_PIXEL_FORMAT_BAYER_GB8:
	{
		emDestPixelFormat = GX_PIXEL_FORMAT_BAYER_GB8;
		break;
	}
	case GX_PIXEL_FORMAT_BAYER_BG10_P:
	case GX_PIXEL_FORMAT_BAYER_BG12_P:
	...
	case GX_PIXEL_FORMAT_BAYER_BG8:
	{
		emDestPixelFormat = GX_PIXEL_FORMAT_BAYER_BG8;
		break;
	}
	case GX_PIXEL_FORMAT_MONO10_PACKED:
	case GX_PIXEL_FORMAT_MONO12_PACKED:
	...
	case GX_PIXEL_FORMAT_MONO8:
	{
		emDestPixelFormat = GX_PIXEL_FORMAT_MONO8;
		break;
	}
	default:
	{
		throw std::invalid_argument("Unexpected PixelFormat!");
		break;
	}
	}

	return emDestPixelFormat;
}
